...
public void OnSQLCheckBanQuery(Handle hParent, Handle hChild, const char[] err, any client)
{
	...
	if (SQL_FetchRow(hQuery))
	{
		...
		g_iSprayUnbanTimestamp[client] = SQL_FetchInt(hQuery, 0);
		SQL_FetchString(hQuery, 1, g_sBanIssuerSID[client], sizeof(g_sBanIssuerSID[]));
		SQL_FetchString(hQuery, 2, g_sBanIssuer[client], sizeof(g_sBanIssuer[]));
		g_iSprayBanTimestamp[client] = SQL_FetchInt(hQuery, 3);
		SQL_FetchString(hQuery, 4, g_sBanReason[client], sizeof(g_sBanReason[]));
	}
}
